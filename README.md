...
| 操作 | 動作 |
|---|---|
| `Ctrl` + ホイール | 表示の拡大 / 縮小（50%〜300%） |
| `Ctrl` + `+` / `Ctrl` + `-` | 同上（キーボード） |
| `Ctrl` + `0` | 表示倍率を 100% に戻す |
| 右下の鉛筆ボタン | 編集モードに切り替え |
| `Ctrl` + `S`（編集中） | 保存してビューに戻る |

倍率はウィンドウを開いている間だけ保持されます（`localStorage`）。
編集の保存やファイルの自動更新で再描画されても倍率は維持され、アプリを閉じると 100% に戻ります。

> **実装メモ**: WebView2 のネイティブズームは pywebview が
> `AreBrowserAcceleratorKeysEnabled` をデバッグ時のみ有効にしているため効きません
> （実測で確認）。CSS `zoom` で自前に実装しています。
> なお CSS `zoom` は `vh` を補正しないため、`vh` を使うエディタの高さは
> `--zoom` カスタムプロパティで割り戻しています。

## Appendix

- 2026年6月1日：編集機能を設けました
- 2026年7月27日：LaTeX数式・画像・GFM記法・脚注・目次などへ対応を拡大しました
- 2026年7月29日：Ctrl+ホイールでの拡大縮小に対応しました